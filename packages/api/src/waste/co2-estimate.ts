...
// fruit and veg are light.
const CO2_PER_KG: Record<TipCategory, number> = {
  viande: 12,
...
export function estimateCo2AvoidedKg(items: ConsumedItemForCo2[]): number {
  ...
  for (const item of items) {
    ...
    total += toKg(item.quantity, item.unit) * factor;
  }
