...
| 13  | Gamification (XP & challenges) | The dossier names the Trashy challenges and the idea of XP but never says how a challenge is measured or completed.                   | Each challenge stores a JSON `rule`; progress is computed from data we already keep; XP is awarded once with a guarded `updateMany`; definitions are seeded on boot; completion is event-driven on `stock.removed`. | The mechanic needed concrete rules and numbers. See below.                                                                                                                      |
